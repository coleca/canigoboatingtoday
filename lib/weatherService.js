...
const NWS_USER_AGENT = 'CanIGoBoatingToday/1.0 (canigoboatingtoday.com, admin@canigoboatingtoday.com)'
...
export async function getTideData(latitude, longitude) {
  try {
    ...
    const tideAPIUrl = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date=${dateString}&end_date=${dateString}&station=${closestStation.id}&product=predictions&datum=MLLW&time_zone=gmt&units=english&format=json`
