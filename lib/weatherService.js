...
export async function getNWSForecast(latitude, longitude) {
  ...
  try {
    const cacheKey = `${FORECAST_CACHE_PREFIX}${latitude.toFixed(2)},${longitude.toFixed(2)}`
    ...
    const [forecastResponse, gridDataResponse] = await Promise.all([
      ...
      fetch(gridDataUrl, { headers: { "User-Agent": NWS_USER_AGENT } })
    ]);
...
function getCachedApiPayload(key, maxAgeMs) {
  if (typeof window === "undefined") return null
...
function cacheApiPayload(key, payload) {
  if (typeof window === "undefined") return
